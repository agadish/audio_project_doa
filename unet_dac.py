...
class UnetDAC(nn.Module):
    def __init__(self, L: int, K: int, M: int, num_classes: int = (180 // 15) + 1, dropout_probability: float = 0.25):
        ...
        # -------
        # input: 572x572x3
        self.e11 = nn.Conv2d(2 * (M - 1), 16, kernel_size=3, padding=1) # output: 570x570x64
        self.e12 = nn.Conv2d(16, 16, kernel_size=3, padding=1) #paoutput: 568x568x64
    ...
    def forward(self, x):
        ...
        xu11 = torch.cat([xu1, xe42], dim=0) # XXX: Changed from dim=1 in all places.
        """
        xu1.shape
        torch.Size([128, 32, 2])
        xe42.shape
        ...
        softmax_res = F.softmax(out, dim=0) # dim=0 refers to the 13 microphones
